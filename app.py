...
function generate_widget_js2(agent_id, branding, brand = "") {
    ...
    (function() {
        ...
        if (expiry && Date.now() > parseInt(expiry)) {
            localStorage.removeItem("convai_form_submitted");
        }
        ...
        window.addEventListener('DOMContentLoaded', () => {
            // Clear expired cache again for safety (in case localStorage mutated after script loaded)
            const expiry2 = localStorage.getItem("convai_form_submitted");
            if (expiry2 && Date.now() > parseInt(expiry2)) {
                localStorage.removeItem("convai_form_submitted");
            }
            ...
            document.getElementById('visitor-form').addEventListener('submit', function(e) {
                ...
                localStorage.setItem("convai_form_submitted", (Date.now() + 120000).toString()); // 2 min cache
